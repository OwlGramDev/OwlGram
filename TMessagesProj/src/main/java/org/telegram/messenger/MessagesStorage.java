...
public class MessagesStorage extends BaseController {
    ...
    private void updateDbToLastVersion(int currentVersion) throws Exception {
        ...
        if (version == 64) {
            database.executeFast("CREATE TABLE IF NOT EXISTS dialog_filter_owl(id INTEGER PRIMARY KEY, ord INTEGER, unread_count INTEGER, flags INTEGER, title TEXT)").stepThis().dispose();
    ...
    private void loadDialogFilters() {
        storageQueue.postRunnable(() -> {
            try {
                ...
                SQLiteCursor filtersCursor = database.queryFinalized("SELECT id, ord, unread_count, flags, title, emoticon FROM dialog_filter_owl WHERE 1");
                ...
                if (!hasDefaultFilter) {
                    ...
                    SQLitePreparedStatement state = database.executeFast("REPLACE INTO dialog_filter_owl VALUES(?, ?, ?, ?, ?, ?)");
    ...
    public void saveDialogFiltersOrderInternal() {
        try {
            SQLitePreparedStatement state = database.executeFast("UPDATE dialog_filter_owl SET ord = ?, flags = ? WHERE id = ?");
            for (int a = 0, N = dialogFilters.size(); a < N; a++) {
                MessagesController.DialogFilter filter = dialogFilters.get(a);
                state.requery();
